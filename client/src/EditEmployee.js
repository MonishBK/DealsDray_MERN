...
const EditEmployee = () => {
  ...
  const [btnStatus, setBtnStatus] = useState(false);
    ...
    let [courseinfo, setCourseInfo] = useState({
      ...
      const FetchEmp = async () => {
        try {
          ...
          const res = await fetch(`/employee-data/${id}`, {
            method: "GET",
            headers: {
              Accept: "application/json",
              "Content-Type": "applictaion/json",
        ...
        setImgload(data.f_Image)
        setCourseInfo({courses:data.f_Course})
    ...
    const PostData = async (e) =>{
        ...
        let { f_Name, f_Email, f_Mobile, f_Designation, f_Gender, f_Course, f_Image}= employeeData
    ...
    if(!f_Name  &&  !f_Email &&  !f_Mobile && !f_Designation  && !f_Gender && courseinfo.courses.length === 0 && !Imgload){
        ...
        const res = await fetch(`/edit-employee-data/${id}`, {
            method: "PUT",
            headers: {
